...
if [ "$1" == "test" ]; then
  ...
  java --module-path lib/javafx/"$os" --add-modules javafx.controls,javafx.fxml,javafx.media -jar ./dist/"$2"/g06-"$2".jar
else
  java --module-path lib/javafx/"$os" --add-modules javafx.controls,javafx.fxml,javafx.media -cp "$CLASS_PATH:./compile/build" be.ac.ulb.infof307.g06.Main
fi
